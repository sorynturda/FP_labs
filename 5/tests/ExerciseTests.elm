      ...
      , test "Second example" <| 
          \_ -> 
            let 
              ...
              user3 = makeUser "jacob14" "jacobh@yahoo.com" "Jacob" "Hunt" (Just "345870481")
            in
              Expect.equal [] <| usersWithPhoneNumbers [user2]
      ]
    , describe "Exercises.changePreferenceToDarkTheme"
      [ skip <| test "First example" <| 
          \_ -> 
            let 
            ...
            in
              Expect.equal [config1Dark, config2, config3] <| changePreferenceToDarkTheme [config1, config1Dark ,config2, config3]
      , skip <| test "Second example" <|
          \_ -> 
            let
              config1 = AccountConfiguration Theme.Light True False
            in
              Expect.equal [] <| changePreferenceToDarkTheme [config1]
      ]
    ]
